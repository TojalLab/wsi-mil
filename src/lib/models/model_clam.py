...
class CLAM_SB(nn.Module):
    def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        self.size_dict = {"small": [1024, 512, 256], "med": [768, 384, 192], "big": [1024, 512, 384], 'big2': [2048, 1024, 512]}
    ...
    def relocate(self, device):
        ...
        #print('relocated to '+str(device))
        if device.type == 'cuda':
            #self.instance_loss_fn = self.instance_loss_fn.to(device) # XXX doesnt work
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        if instance_eval:
            results_dict = {'instance_loss': total_inst_loss, 'inst_labels': torch.cat(all_targets),
            'inst_preds': torch.cat(all_preds)}
        else:
            results_dict = {}
        if return_features:
